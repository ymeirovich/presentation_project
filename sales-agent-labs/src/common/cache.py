...
def get(namespace: str, key: str, *, ttl_secs: Optional[float] = None, root: Path = DEFAULT_STATE_DIR) -> Optional[dict]:
...
def set(namespace: str, key: str, obj: dict, *, root: Path = DEFAULT_STATE_DIR) -> None:
    ...
    with tempfile.NamedTemporaryFile("w", delete=False, dir=str(dirp), encoding="utf-8") as tf:
        json.dump(obj, tf, ensure_ascii=False, indent=2)
        tmpname = tf.name
    os.replace(tmpname, final)  # atomic on POSIX

def llm_key(report_text: str, max_bullets: int, max_script_chars: int, model: str) -> str:
    payload = f"llm|{model}|{max_bullets}|{max_script_chars}|{report_text}"
    return _sha256_hex(payload)

def imagen_key(prompt: str, aspect: str, size: str, model: str, share_public: bool) -> str:
